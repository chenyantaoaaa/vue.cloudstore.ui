    ...
    <metro_page>
        ...
        <metro_page_box>
            <metro_page_box_body class="edit_padding">
                <pm_form_render :model="formModel" :rules="rules" ref="inForm" :entity='mainEntity' v-bind:formReadOnly="formReadOnly">
                    <pm_form_item row="1" labletext="过户单号" name="transferNo" :span="5" xtype="text" lableWidth="70px" :readOnly="true" ></pm_form_item>
                    <pm_form_item row="1" labletext="过户单状态" name="transferStatus" :span="6" xtype="select" lableWidth="100px" :readOnly="true"></pm_form_item>
                    <pm_form_item row="1" labletext="过户人" name="auditorName" :span="8" xtype="text" :readOnly="true" lableWidth="70px"></pm_form_item>
                    <pm_form_item ref= "form_transferTime" row="1" labletext="过户时间" name="transferTime" :span="5" xtype="date" lableWidth="100px" :pickerOptions='pickerOptions'></pm_form_item>
                    <pm_form_item ref= "form_oldCst" row="2" labletext="原货主" required name="oldCstId" :span="5" xtype="search_select_input" lableWidth="70px"></pm_form_item>
                    <pm_form_item ref= "form_newCst" row="2" labletext="新货主" name="newCstId" :span="6" xtype="search_select_input" :alllowCreate="true" autoCreateField="createCstName" lableWidth="100px"></pm_form_item>
                    <pm_form_item row="2" labletext="制单人" name="creatorName" :span="4" xtype="text" lableWidth="70px" :readOnly="true" ></pm_form_item>
                    <pm_form_item row="2" labletext="制单时间" name="createTime" :span="4" xtype="datetime" lableWidth="70px" :readOnly="true" ></pm_form_item>
                    <pm_form_item ref="form_cstInTimeType" row="2" labletext="货主入库时间" name="cstInTimeType" :span="5" xtype="select" lableWidth="100px"  ></pm_form_item>
                    <pm_form_item ref= "form_remark" row="4" labletext="备注" name="remark" :span="24" xtype="textarea" lableWidth="70px"></pm_form_item>
                </pm_form_render>
               <!-- toolbar工具条部分 -->
                <pm_tool_bar>
                    <pm_toolButton ref="btn_selectProduct" btnName="选择明细" btnIcon="el-icon-sold-out" :btnClickFunc="getStock"></pm_toolButton>
                    <pm_toolButton ref="btn_remove" btnName="移除" btnIcon="el-icon-remove" :btnClickFunc="deleteRows"></pm_toolButton>
                </pm_tool_bar>
            </metro_page_box_body>
        </metro_page_box>
        ...
        <metro_page_box>
            <!-- body内容区域 -->
            <metro_page_box_body>
                <pm_table ref="pmTable" tableKey="NoteTransferEdit-pmTable" :dataSource="formModel.aftList" :config="config" :bottomHeight="170">
        ...
        <metro_page_box>
            <metro_page_box_body>
                <pm_tool_bar :noBackground="true">
                    <pm_toolButton ref='btn_save' btnName ="保存" btnIcon ="el-icon-circle-plus-outline" :btnClickFunc ='saveInfo'></pm_toolButton>
                    <pm_toolButton ref='btn_saveAndAudit' btnName ="保存并审核" btnIcon ="el-icon-circle-plus-outline" :btnClickFunc ='saveAndAuditInfo'></pm_toolButton>
                    <pm_toolButton ref="btn_print" btnName ="打印" btnIcon ="el-icon-printer" :btnClickFunc ='printInfo'></pm_toolButton>
                    <pm_toolButton ref='btn_settle' btnName ="结算" btnIcon ="pm-font pm-icon-ERP_xieyibianhao" :btnClickFunc ='settleInfo'></pm_toolButton>
...
import { error } from 'util';
    ...
    export default {
        components: {metro_page, metro_page_box, metro_page_box_tool_bar,
            metro_page_box_body, metro_pageheader,pm_search,pm_form_item,pm_toolButton, pm_tool_bar,pm_table, pm_column,pm_pagination,noteTransferPrint},
        data: function() {
            return {
                dataSource: [],
        ...
        methods: {
            ...
            setCostTime(){
                ...
                if(this.formModel.cstInTimeType == 1){
                    this.formModel.aftList.forEach(item => item.costTime = item.whsinTime);
                }
                //否则 货主入库时间为主单上的过户时间
            ...
            getStock: function(filter, callback) {
                ...
                var $this = this;
                this.$commonUtil.selProduct(this, {cstId:this.formModel.oldCstId}, function(stocks) {
                    var packNoList = [];
                    stocks.forEach(a => {
            ...
            getFormModel:function(){
                ...
                if (entity.method == operTypeEnum.OPER_ADD) {
                    ...
                    let transferTime = util.formatDate.format(new Date(new Date().toLocaleDateString()),"yyyy-MM-dd hh:mm:ss")
            ...
            initFormState: function() {
                switch(this.formModel.method){
                    ...
                    case operTypeEnum.OPER_EDIT:{
                        ...
                        this.formModel.preList = this.$commonUtil.deepClone(this.formModel.aftList);
                        this.config.allowEdit = true;
                        this.$refs["btn_settle"].setDisabled(true);
                        break;
                    }
            ...
            saveInfo: function() {
                ...
                if (this.formModel.method == operTypeEnum.OPER_ADD) {
                    //新增
                    this.baseSave("whsTransfer/addTransferNote", submitModel, "新增过户单成功", data => {
                        $this.save2Update(data);
                    });
                }
                else if(this.formModel.method == operTypeEnum.OPER_EDIT) {
                    //修改
                    this.baseSave("whsTransfer/updateTransferNote", submitModel, "修改过户单成功", data => {
                        $this.save2Update(data);
                    });
                }
                else if(this.formModel.method == operTypeEnum.OPER_ADJUST){
                    //修改
                    this.baseSave("whsTransfer/updateTransferNote", submitModel, "调整过户单成功");
                }
            },
            //保存后跳到修改界面
            save2Update: function(data){
                ...
                this.$commonUtil.tab.updateTitle(this, "编辑过户单[" + data.transferNo + "]");
            },
            //审核后跳到查看界面
            audit2Check: function(data){
                ...
                this.$commonUtil.tab.updateTitle(this, "查看过户单[" + data.transferNo + "]");
            },
            //基础保存方法
            ...
            getEntity: function() {
                ...
                if(this.formModel.createCstName){
                    ...
                    submitModel.totalGrossWeight = submitModel.aftList.sum(a => a.grossWeight);
                    submitModel.totalPoundWeight = submitModel.aftList.sum(a => a.poundWeight);
                    return submitModel;
                }
            },
            createCst: function(callback){
                let _this = this;
                this.$confirm(this.formModel.createCstName+"该货主不存在！是否需要自动创建","提示",{
            ...
            saveAndAuditInfo: function(){
                ...
                var $this = this;
                this.baseSave("whsTransfer/SaveAndAuditTransferNote", submitModel, "保存并审核过户单成功", data => {
            ...
            printInfo: function(){
                ...
                this.$commonUtil.getDetailEntity("whsTransfer/getTransferDetail",
                    {id: info.id},
                    info,
                    function(entity){
